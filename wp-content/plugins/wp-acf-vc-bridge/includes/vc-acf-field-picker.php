...
class VcAcfFieldPicker {
  ...
  public function init() {
    ...
    /*
      ...
      Since 1.2.6
    */
    $is_vc_frontend_editor_request = function_exists( 'vc_post_param' ) ? in_array( vc_post_param( 'action' ), array( 'vc_edit_form', 'vc_load_shortcode' ) ) || ( 'true' === vc_get_param( 'vc_editable' ) ) || ( 'vc_inline' === vc_get_param( 'vc_action' ) ) : false;
  ...
  public function shortcode( $atts, $content = null ) {
    $sanitized_atts = shortcode_atts( $this->defaults, $atts, self::NAME );
    ...
    */
    ...
    $field_object = $this->getFieldObject( $field_name, $post_id );
   ...
   *  getFieldObject
   *
   *  Retrieves ACF Field Object additionally steps around WPML issues
   *
   *  @type  function
   *  @date  25/03/17
   *  @since 1.5.18
  ...
  public function getFieldObject( $name, $post_id ) {
    $field_object = get_field_object( $name, $post_id );

    if ( ! is_array( $field_object ) && $post_id && function_exists( 'icl_object_id' ) ) {
      // If WPML is active, try to get field object from original language, because on some setups it fails to retrieve for translated posts

      $object_id = null;
      if ( is_string( $post_id ) && ( FALSE !== strpos( $post_id, '_' ) ) ) {
        $post_id_parts = explode( '_', $post_id );
        $object_id = array_pop( $post_id_parts );
        $object_id = intval( $post_id );
        $object_type = implode( '_' , $post_id_parts );
      }
      ...
      // If we have both ID and type, try to get ID of original language
      if ( $object_id && $object_type ) {
        global $sitepress;
        $object_id = apply_filters( 'wpml_object_id', $object_id, $object_type, false, $sitepress->get_default_language() );

        // Try to get field object in original language
        $field_object = get_field_object( $name, $object_id );
        if ( is_array( $field_object ) ) {
          // Value should be translated
          $field_object[ 'value' ] = get_field( $name, $post_id );
        }
      }
